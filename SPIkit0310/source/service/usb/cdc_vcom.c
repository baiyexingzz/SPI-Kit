...
void Chip_USB_Init(void)
{
	...
	Clock_SetUSBPLLSource(SYSCTL_PLLCLKSRC_MAINOSC);
	/* Setup USB PLL  (FCLKIN = 12MHz) * 4 = 48MHz
	   MSEL = 3 (this is pre-decremented), PSEL = 1 (for P = 2)
	   FCLKOUT = FCLKIN * (MSEL + 1) = 12MHz * 4 = 48MHz
	   FCCO = FCLKOUT * 2 * P = 48MHz * 2 * 2 = 192MHz (within FCCO range) */
	Clock_SetupUSBPLL(3, 1);

	/* Powerup USB PLL */
	SYSCTL_vPowerUp(SYSCTL_POWERDOWN_USBPLL_PD);
	...
	CLOCK_vEnablePeriphClock(SYSCTL_CLOCK_USB);
	/* power UP USB Phy */
	SYSCTL_vPowerUp(SYSCTL_POWERDOWN_USBPHY_PD);
	/* Reset USB block */
	SYSCTL_vPeriphReset(RESET_USB);
}
